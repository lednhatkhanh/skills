...
2. Cover package design, API boundaries, naming, error semantics, context usage, concurrency safety, testing strategy, and module/version management.
3. Include version-aware guidance for modern Go features and stdlib APIs; avoid suggesting features newer than the module's target Go version.
4. Include clear workflow steps, non-negotiable rules, quality gates, and output expectations.
5. Structure for progressive disclosure: concise SKILL.md + focused references.
6. Include security and reliability practices: race detection, fuzzing, vuln scanning, and safe defaults.
7. Include observability and operability guidance: structured logging, cancellation/timeout boundaries, and failure transparency.
8. Avoid speculative future-version claims; verify version-specific statements against official release notes.
9. Prefer minimal, targeted edits when updating an existing skill.
...
4. Explicit assumptions (target Go version, compatibility constraints, project type if inferred).
